...
class MagePsycho_Massimporterpro_Model_Import_Adapter
{
    ...
    public static function factory($type, $options = null)
    {
        if (!is_string($type) || !$type) {
            Mage::throwException(Mage::helper('massimporterpro')->__('Adapter type must be a non empty string'));
        }
        ...
        if (! $adapter instanceof MagePsycho_Massimporterpro_Model_Import_Adapter_Abstract) {
            Mage::throwException(
                Mage::helper('massimporterpro')->__('Adapter must be an instance of MagePsycho_Massimporterpro_Model_Import_Adapter_Abstract')
            );
        }
        return $adapter;
    }
