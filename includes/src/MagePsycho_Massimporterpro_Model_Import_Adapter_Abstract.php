...
abstract class MagePsycho_Massimporterpro_Model_Import_Adapter_Abstract implements SeekableIterator
{
    ...
    final public function __construct($options = array())
    {
        ...
        if (!is_string($source)) {
            Mage::throwException(Mage::helper('massimporterpro')->__('Source file path must be a string'));
        }
        if (!is_readable($source)) {
            Mage::throwException(Mage::helper('massimporterpro')->__("%s file does not exists or is not readable", $source));
        }
        $this->_source = $source;
        ...
        if (is_array($this->_colNames) && !empty($this->_colNames)) {
            ...
            if (count(array_unique($this->_colNames)) != $this->_colQuantity) {
                Mage::throwException(Mage::helper('massimporterpro')->__('Column names have duplicates'));
            }
        } else {
            Mage::throwException(Mage::helper('massimporterpro')->__('Column names is empty or is not an array'));
        }
    }
    ...
    {
        Mage::throwException(Mage::helper('massimporterpro')->__('Not implemented yet'));
    }
