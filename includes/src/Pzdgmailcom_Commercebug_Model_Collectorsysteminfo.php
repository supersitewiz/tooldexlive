...
*/
class Pzdgmailcom_Commercebug_Model_Collectorsysteminfo extends Pzdgmailcom_Commercebug_Model_Observingcollector
{
    protected $_items;
    public function collectInformation($observer)
    {
        ...
        {
            Pzdgmailcom_Commercebug_Model_Crossareaajax::validateControllers();
        }
        
        $system_info->ajax_paths = new stdClass;
        $system_info->form_key = $this->getShim()->getSingleton('core/session')->getFormKey();
        foreach(Pzdgmailcom_Commercebug_Model_Crossareaajax::getAjaxActionNames() as $action)
        {
